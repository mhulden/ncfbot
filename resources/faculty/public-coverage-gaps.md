...
| Step-by-step inside SES for entering a narrative evaluation | Partly public — the published step names are in `faculty-deadlines-and-systems`; the screens after login are not | Office of the Registrar |
| Where SES or another system appears inside myNCF; any dashboard, tile, menu, tab, or button not listed in `faculty-deadlines-and-systems` | Not public — no approved source describes it, and a hedged guess counts as invented | Say the step continues behind login; Office of the Registrar for evaluation screens |
